...
COURSE_URL = (os.getenv("COURSE_URL", "") or "").strip()
...
def is_admin(update: Update) -> bool:
    try:
        return ADMIN_TELEGRAM_ID and str(update.effective_user.id) == ADMIN_TELEGRAM_ID
    except Exception:
        return False
...
def raw_text(update: Update) -> str:
...
async def start_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Нажми кнопку «Соглашаюсь».")
    return ASK_CONSENT

async def got_consent(update: Update, context: ContextTypes.DEFAULT_TYPE):
    text = raw_text(update).lower()
    if "соглаша" not in text:
        await update.message.reply_text("Пожалуйста, нажми «Соглашаюсь».", reply_markup=CONSENT_KB)
        return ASK_CONSENT
    await update.message.reply_text("Ок. Введи ФИО (например: Иванов Иван Иванович).", reply_markup=ReplyKeyboardRemove())
    return ASK_FIO
...
async def got_fio(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text("Пол ребёнка?", reply_markup=GENDER_KB)
    return ASK_GENDER

async def got_gender(update: Update, context: ContextTypes.DEFAULT_TYPE):
    g = raw_text(update).lower()
    if g.startswith("м"):
        gender = "boy"
    elif g.startswith("д"):
        gender = "girl"
    else:
        await update.message.reply_text("Выбери кнопку: «Мальчик» или «Девочка».", reply_markup=GENDER_KB)
        return ASK_GENDER
    context.user_data["gender"] = gender
    await update.message.reply_text("Укажи e-mail (например: test@example.com).", reply_markup=ReplyKeyboardRemove())
    return ASK_EMAIL

async def got_email(update: Update, context: ContextTypes.DEFAULT_TYPE):
    email = raw_text(update)
    ...
    try:
        _id = add_lead(
            ...
            details="ОПП: согласие получено",
            gender=gender,  # ← пишем в отдельную колонку
        )
        log.info("lead saved: id=%s fio=%s email=%s gender=%s", _id, fio, email, gender)
    except Exception as e:
        log.exception("add_lead failed: %s", e)
...
async def find_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    if not is_admin(update):
        await update.message.reply_text("Команда недоступна.")
        return
    text = raw_text(update)
    m = re.match(r"^/find(@\w+)?\s+(.*)$", text, flags=re.IGNORECASE | re.DOTALL)
...
async def set_status_cmd(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    text = raw_text(update)
    m = re.match(r"^/set_status(@\w+)?\s+(\d+)\s+([A-Za-z_]+)\s*$", text, flags=re.IGNORECASE)
    if m:
        lead_id_s = m.group(2)
        new_status = m.group(3).lower()
    else:
        if len(context.args) < 2:
            await update.message.reply_text(f"Формат: /set_status <id> <{'|'.join(ALLOWED_STATUSES)}>")
            return
        lead_id_s = context.args[0]
        new_status = context.args[1].lower()

    try:
        lead_id_int = int(lead_id_s)
    except ValueError:
        await update.message.reply_text("id должен быть числом.")
        return

    if new_status not in ALLOWED_STATUSES:
        await update.message.reply_text(f"Недопустимый статус. Разрешённые: {', '.join(ALLOWED_STATUSES)}")
        return
